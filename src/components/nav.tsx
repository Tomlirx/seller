...
export function Nav() {
  ...
  return (
    <>
      <header
        className={`fixed inset-x-0 top-0 z-50 h-[88px] transition-colors duration-300 ${
          transparent ? "bg-transparent border-b border-transparent" : "bg-ivory/95 backdrop-blur border-b border-line"
        }`}
      >
      <nav className="mx-auto max-w-[1600px] px-6 sm:px-20 h-full grid grid-cols-3 items-center">
        <Link href="/" className="flex flex-col items-start leading-tight">
          <span className={`font-serif text-xl tracking-wide ${transparent ? "text-ivory-light" : "text-ink"}`}>
            岚玉
          </span>
        ...
        <div className="hidden sm:flex justify-center">
          <Link
            ...
            className={`text-sm tracking-wide hover:text-gold transition-colors ${
              transparent ? "text-ivory-light" : "text-ink"
            }`}
          >
            品类 Collections
          </Link>
        ...
        <div
          className={`flex items-center justify-end gap-4 ${transparent ? "text-ivory-light" : "text-ink"}`}
        >
